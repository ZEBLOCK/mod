...
class GuiSlotServer extends GuiSlot
{
    ...
    private void func_77248_b(int par1, int par2, int par3, int par4, Tessellator par5Tessellator)
    {
        ...
        this.parentGui.drawString(this.parentGui.fontRenderer, I18n.getString("lanServer.title"), par2 + 2, par3 + 1, 0xffffff);
        this.parentGui.drawString(this.parentGui.fontRenderer, lanserver.getServerMotd(), par2 + 2, par3 + 12, 0xffffff);

    }

    private void func_77249_c(int par1, int par2, int par3, int par4, Tessellator par5Tessellator)
    {
        this.parentGui.drawCenteredString(this.parentGui.fontRenderer, "请选择zeblock服务器加入", this.parentGui.width / 2, par3 + 1, 0xffffff);
        String s;
    ...
    private void func_77247_d(int par1, int par2, int par3, int par4, Tessellator par5Tessellator)
    {
        ...
        this.parentGui.drawString(this.parentGui.fontRenderer, serverdata.serverName, par2 + 2, par3 + 1, 0xffffff);
        this.parentGui.drawString(this.parentGui.fontRenderer, serverdata.serverMOTD, par2 + 2, par3 + 12, 0xffffff);
        this.parentGui.drawString(this.parentGui.fontRenderer, serverdata.populationInfo, par2 + 215 - this.parentGui.fontRenderer.getStringWidth(serverdata.populationInfo), par3 + 12, 0xffffff);

        if (flag2)
        {
            String s = EnumChatFormatting.YELLOW + serverdata.gameVersion;
            this.parentGui.drawString(this.parentGui.fontRenderer, s, par2 + 200 - this.parentGui.fontRenderer.getStringWidth(s), par3 + 1, 0xffffff);
        }

        if (!this.parentGui.mc.gameSettings.hideServerAddress && !serverdata.isHidingAddress())
        {
            this.parentGui.drawString(this.parentGui.fontRenderer, serverdata.serverIP, par2 + 2, par3 + 12 + 11, 0xffffff);
        }
        else
        {
            this.parentGui.drawString(this.parentGui.fontRenderer, I18n.getString("selectServer.hiddenAddress"), par2 + 2, par3 + 12 + 11, 0xffffff);
        }
